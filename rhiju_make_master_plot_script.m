...
data_files = {...
    ...
    'Bioanalyzer_2021_Stanford_S2P.csv',...
}
...
k_pred = [];
clf;
modification = repmat({'unmod'},1,length(k_deg));
for i = 1:length(k_deg)
    ...
    if ~isempty(strfind(RNA_type{i}, 'm1PSU' )); modification{i} = 'm1PSU'; continue; end;
    if ~isempty(strfind(RNA_type{i}, 'PSU' )); modification{i} = 'PSU'; continue; end;
    if ~isempty(strfind(RNA_type{i}, '5mC' )); modification{i} = 'm5C'; continue; end;
end
